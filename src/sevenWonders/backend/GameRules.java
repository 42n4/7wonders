...
/**
 * Handles some of the gamerules. It calculates war and updates neighbour
 * dependant guilds.
 * 
 * @author Jenny Norelius & Andreas Jönsson
 */
public class GameRules {
    private static int[] pointsForWin = new int[] { 1, 3, 5 };
	...
	for (int i = 0; i < numPlayers; i++) {
		...
		if (p.wonder.stages[stage] instanceof Shields) {
		    shields[i] += ((Shields) p.wonder.stages[stage])
			    .getShields();
		}
	    }
	}
	...
	for (int i = 0; i < numPlayers; i++) {
		...
		if (nd instanceof BuildingRewardCard) {
		    ...
		    count += players.get(i).wonder.getCurrentLevel();
		    count += players.get(left(i, numPlayers)).wonder
			    .getCurrentLevel();
		    count += players.get(right(i, numPlayers)).wonder
			    .getCurrentLevel();
			...
			if (c instanceof BrownCard || c instanceof GrayCard
