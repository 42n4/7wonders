...
/**
 * GameEngine creates a new game and runs it. It handles all the logic and sends
 * and receives gamepackages from the playerconnections. It currently can only
 * create a 3-player game with one player and 2 NPC.
 * 
...
public class GameEngine {
	...
	for (int era = 1; era <= 3; era++) {
		...
		// calculates paymentoptions for every player and sends data to
		// client
		for (int i = 0; i < numPlayers; i++) {
		    Hand hand = handsWithPaymentOptions[i] = HandGenerator
			    .getHand(hands[i], players, i);
		...
		for (int i = 0; i < numPlayers; i++) {
			...
			p.buildCard(a.card);
			execute(a.card, i);
			po = handsWithPaymentOptions[i].cards.get(a.card).get(
				a.paymentOptionIndex);
			break;
		    case BUILD_WONDER:
			p.wonder.build(era);
			execute(p.wonder.stages[p.wonder.getCurrentLevel() - 1],
				i);
			po = handsWithPaymentOptions[i].wonderPaymentOptions
				.get(a.paymentOptionIndex);
			break;
		    case DISCARD_CARD:
	...
	List<List<Integer>> points = PointCalculator.getPoints(players);
	for (int i = 0; i < numPlayers; i++) {
	    // retries until succesful
